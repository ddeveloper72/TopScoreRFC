    ...
    <div class="event-manager-container">
      ...
      <div class="match-info">
        ...
        <span class="match-date">{{
          data.match.date | date : 'MMM dd, yyyy'
        }}</span>
      </div>
          ...
          <mat-card-content>
            <form [formGroup]="eventForm" class="event-form">
                ...
                <mat-form-field appearance="outline">
                  ...
                  <mat-select formControlName="team" required>
                    <mat-option value="home"
                      >{{ data.match.homeTeam }} (Home)</mat-option
                    >
                    <mat-option value="away"
                      >{{ data.match.awayTeam }} (Away)</mat-option
                    >
                  </mat-select>
                </mat-form-field>
              </div>
              ...
              <mat-form-field appearance="outline" class="full-width">
                ...
                <input
                  matInput
                  formControlName="ourPlayer"
                  placeholder="Player name (for home team events)"
                />
                <mat-hint
                  >Focus on tracking your own team's player
                  performance</mat-hint
                >
              </mat-form-field>
...
export class EventManagerDialogComponent implements OnInit {
  ...
  addEvent(): void {
    if (this.eventForm.valid && !this.isLoading) {
      ...
      eventData.pointsSnapshot =
        (this.data.match.homeScore || 0) + (this.data.match.awayScore || 0);
      ...
      if (this.data.match._id) {
        ...
        this.matchApi.addMatchEvent(this.data.match._id, eventData).subscribe({
          next: (response) => {
            ...
            this.matchStorage.addEventToMatch(
              this.data.match.id || this.data.match._id!,
              response.event
            );
  ...
  deleteEvent(event: MatchEvent, showConfirmation: boolean = true): void {
    ...
    if (eventIndex > -1) {
      ...
      if (this.data.match._id && event._id) {
        this.matchApi
          .deleteMatchEvent(this.data.match._id, event._id)
          .subscribe({
            next: () => {
              this.matchStorage.deleteEventFromMatch(
                this.data.match.id || this.data.match._id!,
                event._id!
              );
              if (showConfirmation) {
        ...
        if (showConfirmation) {
          this.snackBar.open('Event deleted locally!', 'Close', {
            duration: 3000,
          });
        }
      }
